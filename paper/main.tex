...
grid decarbonisation. A four-predictor OLS regression of state-level electricity demand growth
(2022--2024) yields $R^2 = 0.89$ ($n=30$, HC3 robust SEs); data centre
...
(VIF\,=\,5.2). Moran's~I on OLS residuals is $I = -0.17$ ($p > 0.05$),
indicating no statistically significant spatial autocorrelation. Northern
Virginia's DCGSI of 9.9 is the highest nationally, 1.9$\times$ the
primary-market median of 5.2---a result that is robust across 91\% of
...
DCGSI = 9.9, the highest in the study;
...
Dispersed          & 2.8 & 11.3\% [9.8, 12.9]  & 58 & \multicolumn{2}{c}{Various} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
\textbf{194\,gCO$_2$/kWh}---a reduction of 46\% attainable solely
through siting reallocation, without any change to generation mix.
The dominant driver is Northern Virginia's large share (30.6\%)
combined with the SRVC sub-region's 324\,gCO$_2$/kWh intensity;
the Pacific Northwest (NWPP: 136\,gCO$_2$/kWh) would need to
absorb $\approx$8$\times$ its current capacity to fully exploit
its clean-energy advantage.
Fig.~\ref{fig:carbon} presents the per-market breakdown.
\end{sloppypar}

\textbf{Counterfactual limitations.} The 46\% figure represents a
...
drives a DCGSI of 9.9---1.9$\times$ the primary-market median of 5.2---a
result preserved in 91\% of 10{,}000 Monte Carlo weight draws from the
